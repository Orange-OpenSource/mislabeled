...
def test_param_count_linear_model():
    ...
    assert param_count(logreg, None, None) == 2 + 1
    assert param_count(logreg_nobiais, None, None) == 2
    assert param_count(kernel_logreg, None, None) == 1000 + 1
    assert param_count(bagged_logreg, None, None) == 100 * (2 + 1)
    assert param_count(boosted_logreg, None, None) == 100 * (2 + 1)
