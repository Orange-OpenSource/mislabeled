...
def test_proba_self_confidence_is_borned():
    ...
    assert np.all(confidence(probas_pred) <= 1)
    assert np.all(confidence(probas_pred) >= 0)
    assert np.all(confidence(probas_pred, y) <= 1)
    assert np.all(confidence(probas_pred, y) >= 0)


def test_unsupervised_logits_self_confidence_is_abs():
    logits_pred = np.random.normal(size=(1000,))
    np.testing.assert_allclose(confidence(logits_pred), np.abs(logits_pred))
